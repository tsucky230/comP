...
#### 4.1.2 VSCode拡張機能からの書き込みはRPC経由に統合（Phase 3, A7/A9）

v0.11.1 のエージェント別分割以降、TypeScript側の `SessionMemoryManager`（`src/mcp/sessionMemory.ts`）は旧単一ファイル `.comp/session-memory.json` を読み書きし続けており、**誰も読まないファイルへの書き込み**と**誰も書かないファイルからの読み込み**という2つの死んだ経路になっていました（2026-09 のレビューで判明）。具体的には3箇所が影響を受けていました。

- `extension.ts` のファイル変更ウォッチャー: `markStaleForFile` が無意味な書き込みを継続
- `SidebarPanel.ts` の「最終エージェント接続時刻」表示: 常に "Waiting..." のまま
- `commands.ts` の `comp.exportDebugLog` コマンド: 「見つかりません」警告が出るだけ

`SessionMemoryManager` クラスとそのテストは削除し、呼び出し元が Rust daemon へ直接 RPC で委譲する方式に統合しました。

- **`mark_stale` RPC**（`daemon/src/mcp/mod.rs::handle_mark_stale`）: `.comp/session-memory/*.json` を全エージェント分スキャンし、`files` に指定パスを含む呼び出しへ `stale: true` を立てる。ファイルごとに `mark_stale_in_file` が `record_mcp_call` と同じサイドカーロック＋アトミック書込を使うため、対象エージェントのプロセスが同時に書き込んでいても安全。マッチが0件のファイルは書き込み自体を行わない（`tools/list` には載せない内部APIで、AIエージェントからは呼ばれない）。
- **`get_stats` の `last_activity_at`**: `latest_activity_timestamp` が全session-memoryファイル＋全historyファイルを横断して最新タイムスタンプを計算し返す。`SidebarPanel.ts` は既に呼んでいる `getStats()` のレスポンスからこの値を読むだけになり、専用のファイルアクセスが不要になった。
- **`comp.exportDebugLog`**: 単一ファイルという前提が無くなったため、`session_recall` RPCが返す集約済みMarkdownをエクスポート対象に変更（出力拡張子も `.json` → `.md`）。
...
| `agent` | string | 記録したエージェント/LLM（例: `"claude-code"`, `"codex"`）。`#[serde(default = "default_agent")]` により旧データは `"unknown"` として読める |

`session_recall` ツールは **全セッション・全エージェント**（複数デーモン再起動にわたる全エントリ）を横断して検索します。
...
#### 4.2.1 複数 LLM 書き込みとロック

複数の独立した `comp-daemon` プロセスが同じ JSONL に追記できるよう、`append_history_line`（`daemon/src/mcp/mod.rs`）が以下を保証します。
...
- **呼び出し口（Phase 3, B5）**: `comp-daemon compact-history <workspace_root>` CLI サブコマンドが `.comp/history/*.jsonl` を全件走査し、`dedup_exact_duplicate_lines`（完全一致する重複行のみ除去し、各行の最初の出現を残す）を `compact_fn` として適用します。年齢ベースの破棄や要約といった非可逆・非保守的な方針は意図的に採用していません — 重複排除は「既にファイル内のどこかに同じ内容がある行を消す」だけなので、情報を失うリスクがありません。
