...
- **`session_recall` の破損可視化・history修復・`doctor` CLI・degraded起動・mutex poisoned対応**（`daemon/src/mcp/mod.rs`、`daemon/src/main.rs`）: `session_recall` が読めなかった session-memory ファイル／history 行の件数を `⚠ N file(s)/M line(s) could not be read` として出力に明示。`repair_history_file` が jsonl の破損行を `.jsonl.quarantine` へ追記退避しつつ正常行のみ書き戻し、`compact_history_file` と共有する `write_generational_backup` で `.bak` を世代管理。`comp-daemon doctor [--repair]` CLI サブコマンドを新設（フル起動を伴わない軽量パスで3ストアを検査・修復。`index.db` は `PRAGMA busy_timeout` 未設定のままだと他プロセスが使用中で一時的にロックされているだけの健全なDBを破損と誤判定しうる欠陥を自己レビューで検出し、busy/locked と真の破損を区別する `classify_quick_check_result` で対応）。`AppState::new` 失敗時は即終了せず `run_degraded_server` が最小限のMCPサーバーを起動し失敗理由を返す。DB Mutex の poisoned 検知（`is_poisoned_mutex_error`）をディスパッチループに配線し、検知時はプロセスを終了してクライアントに再起動させる
- **session-memory の読み書きをVSCode拡張機能からRust daemonへのRPC委譲に統合**（`daemon/src/mcp/mod.rs`、`src/extension.ts`、`src/ui/SidebarPanel.ts`、`src/ui/commands.ts`、`src/daemon/DaemonManager.ts`）: v0.11.1のエージェント別分割以降、TypeScript側 `SessionMemoryManager`（削除済み）が旧単一ファイル `.comp/session-memory.json` を読み書きし続け、ファイル変更時のstaleマーク・サイドバーの「最終エージェント接続時刻」・`comp.exportDebugLog` の3箇所が実質的に機能しなくなっていた欠陥を修正。新設 `mark_stale` RPC（`record_mcp_call` と同じロック・アトミック書込discipline）と `get_stats` の `last_activity_at` フィールドに統合し、`exportDebugLog` は `session_recall` の集約済みMarkdownをエクスポート対象に変更
- **Stop hookフォールバックのspill方式化**（`.claude/hooks/history-record.sh`、`daemon/src/mcp/mod.rs`）: `comp-daemon` バイナリ不在時のフォールバックが共有jsonlへ `fs.appendFileSync` で直接追記していた（ロック機構を迂回する「第3の書き手」という自己矛盾）のを、一意な `spill-<pid>-<epoch_ms>-<rand>.jsonl` への単独書き込みに変更。`append_history_line` が通常のロック付き追記のたびに `merge_spill_files` で自動的に本体へ取り込み・削除する
- **`compact_history_file` の呼び出し口を配線**（`daemon/src/mcp/mod.rs`）: `comp-daemon compact-history <workspace_root>` CLI サブコマンドを新設し、`.comp/history/*.jsonl` 全件に完全重複行排除（`dedup_exact_duplicate_lines`）を適用可能に
